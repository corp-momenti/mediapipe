...
absl::Status RunMPPGraph() {
  ...
  while (grab_frames) {
    ...
    if (!geometry_poller.Next(&geometry_packet)) {
        ...
        std::string output_data;
        absl::StrAppend(&output_data, geometry_packet.Timestamp().Value(), ",");
    }

    const auto& multiFaceGeometry = geometry_packet.Get<std::vector<::mediapipe::face_geometry::FaceGeometry>>();
